...
static const string TORSO_LIFT_JOINT_NAME = "torso_lift_joint";
    ...
    : it_(nh_)
{
    ...
    torsoPub_ = nh_.advertise<trajectory_msgs::JointTrajectory>("/torso_controller/command", 1);
    ...
    ros::ServiceServer stopService = nh_.advertiseService("stop_person_detection", &ImageConverter::Stop, this);
...
void ImageConverter::liftTorso(double torsoJointRads)
{
    trajectory_msgs::JointTrajectory torsoTrajectory;
    torsoTrajectory.joint_names.resize(1);
    torsoTrajectory.joint_names[0] = TORSO_LIFT_JOINT_NAME;

    torsoTrajectory.points.resize(1);

    torsoTrajectory.points[0].positions.resize(1);
    torsoTrajectory.points[0].positions[0] = torsoJointRads;

    torsoTrajectory.points[0].velocities.resize(1);
    torsoTrajectory.points[0].accelerations.resize(1);
    torsoTrajectory.points[0].effort.resize(1);
    torsoTrajectory.points[0].time_from_start = ros::Duration(2.0, 0.0);

    torsoPub_.publish(torsoTrajectory);

    torsoJointRads_ = std::numeric_limits<double>::max();
   
    ros::Rate rate(10);
    
    while (ros::ok())
    {
        if ((torsoJointRads_ < std::numeric_limits<double>::max()) && 
            (abs(torsoJointRads_ - torsoJointRads) < 0.1))
        {
           return;
        }
...
void ImageConverter::jointStatesTopicCb(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    {
        if (msg->name[i] == HEAD_1_JOINT_NAME)
            angleRads_ = msg->position[i];
        else if (msg->name[i] == TORSO_LIFT_JOINT_NAME)
            torsoJointRads_ = msg->position[i];
    }
}
